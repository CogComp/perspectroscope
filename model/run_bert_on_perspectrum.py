...
from .run_classifier import MrpcProcessor, logger, convert_examples_to_features, \
...
class BertBaseline:
    ...
    def predict(self, sent1, sent2):
        ...
        with torch.no_grad():
            input_ids_tensor = torch.tensor([feature.input_ids]).to(self._device)
            segment_ids_tensor = torch.tensor([feature.segment_ids]).to(self._device)
            input_mask_tensor = torch.tensor([feature.input_mask]).to(self._device)
    ...
    def predict_batch(self, sent_list: List, batch_size=20):
        ...
        for i, (sent1, sent2) in enumerate(sent_list):
            ...
            if len(list_of_examples) > batch_size or i + 1 == len(sent_list):
                ...
                with torch.no_grad():
                    input_ids_tensor = torch.tensor([f.input_ids for f in features]).to(self._device)
                    segment_ids_tensor = torch.tensor([f.segment_ids for f in features]).to(self._device)
                    input_mask_tensor = torch.tensor([f.input_mask for f in features]).to(self._device)
